...
srun -p ${PARTITION} \
    ...
    python -u ./tools/train.py ./configs/mic/viperHR2csHR_mic_hrda.py --launcher="slurm" --l-warp-lambda=1 --l-warp-begin=0 --l-mix-lambda=0 --warp-cutmix True --bottom-pl-fill True --max_confidence True --no-masking True --optimizer $OPTIM --lr-schedule $LR_SCHED --lr=$LR --total-iters=$ITERS --work-dir="./work_dirs/max_confidence/${FULL_JOB_NAME}" --wandbid=${FULL_JOB_NAME}