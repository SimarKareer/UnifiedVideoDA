...
srun -p ${PARTITION} \
    ...
    python -u ./tools/train.py ./configs/mic/viperHR2csHR_mic_hrda.py --launcher="slurm" --l-warp-lambda=-1 --l-mix-lambda=1 --no-masking True --optimizer $OPTIM --lr-schedule $LR_SCHED --lr=$LR --total-iters=$ITERS --work-dir="./work_dirs/1gpu_baseline/${FULL_JOB_NAME}" --wandbid=${FULL_JOB_NAME}