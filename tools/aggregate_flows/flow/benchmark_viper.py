...
common_rvc_subfolder = os.path.realpath(
    os.path.join(
        os.path.dirname(os.path.realpath(os.path.abspath(__file__))), "../../common/"
    )
)
...
class VIPER(Benchmark):
    ...
    def CanConvertOriginalToFormat(self, dataset_format):
        return isinstance(dataset_format, MiddleburyFormat) or isinstance(
            dataset_format, Kitti2015Format
        )

    def ConvertToKittiFormat(
        self, unpack_dir_path, metadata_dict, training_dir_path, test_dir_path
    ):
        """
			unpack: where the OG files are
			training_dir_path: whereu want the training data to go
			test_dir_path: where u want the test data to go
		"""
        ...
        # # convert images
        # for subset_src_dir, subset_dst_dir in [
        #     (src_train_path, training_dir_path),
        #     (src_test_path, test_dir_path),
        # ]:
        ...
        #     sequences = [
        #         d
        #         for d in os.listdir(img_src_dir)
        #         if os.path.isdir(os.path.join(img_src_dir, d))
        #     ]
        ...
        #         images = [
        #             f
        #             for f in os.listdir(seq_src_dir)
        #             if f.endswith(".jpg") or f.endswith(".png")
        #         ]
        #         for img in images:
        #             frame = int(img[4:-4])
        #             shutil.copy(
        #                 os.path.join(seq_src_dir, img),
        #                 os.path.join(
        #                     seq_dst_dir, "%s_%04d.%s" % (seq, frame, img[-3:])
        #                 ),
        #             )
        #             pass
        #         pass
        ...
        sequences = [
            d
            for d in os.listdir(flow_src_dir)
            if os.path.isdir(os.path.join(flow_src_dir, d))
        ]
        for seq in sequences:
            ...
            for flow in flows:
                ...
                invalid = np.logical_or(
                    ...
                    np.logical_or(np.isinf(u), np.isinf(v)),
                )
                u[invalid] = 0
                v[invalid] = 0
                invalid = np.logical_or(
                    invalid, np.logical_or(np.abs(u) >= 512, np.abs(v) >= 512)
                )
                u[invalid] = 0
                v[invalid] = 0
    ...
    def ConvertToKittiFormat2(self, old_path, new_path):
		# convert flow
        flow_src_dir = os.path.join(old_path, "flow")
        flow_dst_dir = os.path.join(new_path, "flow_occ")
        ...
        for subset_src_dir, subset_dst_dir in [
            ...
            for seq in sequences:
                ...
                for img in images:
                    ...
                    shutil.copy(
                        os.path.join(seq_src_dir, img),
                        os.path.join(seq_dst_dir, "frame_%04d.%s" % (frame, img[-3:])),
                    )
                    pass
                pass
    ...
    def ConvertOriginalToFormat(
        ...
        if isinstance(dataset_format, MiddleburyFormat):
            self.ConvertToMiddleburyFormat(
                unpack_dir_path, metadata_dict, training_dir_path, test_dir_path
            )
